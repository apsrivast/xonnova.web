...
* 
*/
class TransferMexEarning extends CI_Controller
{
	...
	function transferStoreCreditByEarning(){
		...
		$sendUserCurrency = $this->mdl_common->sponsorCountry($sendUserId);
		
		if($sendUserCurrency != "MEX"){
				$data = array('message'=>'To transfer to this user, please use US wallet');
				echo json_encode($data);
				return false;
		}
		...
		if($lastName != $_POST['last_name']){
			...
			if($_POST['credit'] <= $userStoreCredit){
				...
				$Arr = array(
						...
						'wallet_type'=>'2',
						'store_type'=> '2',
						'message'=>"transfer MEX Store Credit to user ".$toUserName,
					);
				...
				$Arr = array(
						...
						'wallet_type'=>'1',
						'store_type'=> '2',
						'message'=>"transfer MEX Store Credit by user  ".$UserName,
					);
				$this->db->insert('mxtopup_report_info',$Arr);
				$existUser = count($this->mdl_common->allSelects('SELECT * FROM mxtopup_report_user_map_info WHERE user_id='.$sendUserId));
	...
	function buyStoreCreditByEarning(){
		...
		if(!empty($creditAmount) && $_POST['credit'] <= $totalBalance){
			...
			$Arr = array(
				...
				'wallet_type'=>'1',
				'store_type'=> '1',
				'message'=>"Buy Store Credit",
			);
			$this->db->insert('mxtopup_report_info',$Arr);

			$earning_details_by_user = array(
					'user_id'=>$this->session->userdata('user_id'),
					'description'=>'Buy Mexico Store Credit',
					'amount'=> -$_POST['credit'],
			...
			$existUser = count($this->mdl_common->allSelects('SELECT * FROM mxtopup_report_user_map_info WHERE user_id='.$this->session->userdata('user_id')));
			if(empty($existUser) && $existUser == 0){
				$insertwallete = array(
					'user_id'=>$this->session->userdata('user_id'),
					'admin_id'=>1,
	...
	function getBuyStoreCreditList(){
		header('Content-Type: application/json');
		$userId = $this->session->userdata('user_id');
		$contentData = $this->mdl_common->allSelects('SELECT a.*,b.* FROM user_master as a RIGHT JOIN mxtopup_report_info as b on b.user_id=a.user_id WHERE b.store_type="1" AND b.wallet_type="1" AND b.user_id='.$userId.' ORDER BY wallet_credited_at DESC');
		if(!empty($contentData)){
			foreach ($contentData as $key => $value) {
				$arr[]=$value;
			}
