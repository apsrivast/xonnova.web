...
class Rank extends CI_Controller
{
	...
	function getDownlineForAdminList($userId){
		$getTwo = $this->mdl_common->allSelects('SELECT user_id From user_master WHERE sponsor_id = '.$userId);
		   if(isset($getTwo) && !empty($getTwo)){
		   		$user_level = $this->mdl_common->getLelevel($userId);
		    	 $list = $this->mdl_common->getDownlineList($userId, $user_level);
	...
	function getDownlineList(){
		...
		$getTwo = $this->mdl_common->allSelects('SELECT user_id From user_master WHERE sponsor_id = '.$userId);
		   if(isset($getTwo) && !empty($getTwo)){
		    	 $list = $this->mdl_common->getDownlineList($userId);
	  				echo json_encode($list); 
		    }
	}
	...
	function getUserRank(){
		...
		if($level1 == 1){
			...
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>$levelExitPrice,'total'=>$total,'labelText'=>'Total QV','requiredDiff'=>'Required QV to complete level ', 'labelTextRequired'=>'Total QV to complete level ');
			...
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>2,'total'=>$total,'labelText'=>'Total Team Lead Member', 'requiredDiff'=>'Required Team Lead  Member to complete level', 'labelTextRequired'=>'Total Team Lead Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 7){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total Director Member', 'requiredDiff'=>'Required Director  Member to complete level',  'labelTextRequired'=>'Total Director  Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 8){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total Regional Member', 'requiredDiff'=>'Required Regional  Member to complete level',  'labelTextRequired'=>'Total Regional  Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 9){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total National Member', 'requiredDiff'=>'Required International Member to complete level', 'labelTextRequired'=>'Total National  Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 10){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total International Member', 'requiredDiff'=>'Required International  Member to complete level',  'labelTextRequired'=>'Total International  Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 11){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total VP Member',  'requiredDiff'=>'Required President Member to complete level', 'labelTextRequired'=>'Total President Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 12){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total P Member', 'requiredDiff'=>'Required Ambasdor Member to complete level',  'labelTextRequired'=>'Required Ambasdor Member to complete level');
	...
	function getUserRank1(){
		...
		if($level1 == 1){
			$total = $rightUserTotal + $leftUserTotal;
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>$levelExitPrice,'total'=>$total,'labelText'=>'Total BV', 'labelTextRequired'=>'Required BV to complete level ');
			...
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>2,'total'=>$total,'labelText'=>'Total Team Lead Member', 'labelTextRequired'=>'Required Team Lead Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 7){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total Director Member', 'labelTextRequired'=>'Required Director Member Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 8){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total Regional Member', 'labelTextRequired'=>'Required Regional Member Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 9){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total National Member', 'labelTextRequired'=>'Required National Member Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 10){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total International Member', 'labelTextRequired'=>'Required International Member Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 11){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total VP Member', 'labelTextRequired'=>'Required VP Member Member to complete level');

			echo json_encode($data);
		}elseif($level1 == 12){
			$total = $this->countUserRank($userId, $level);
			$currentRank = $this->mdl_common->getCurrentRankById($level);
			$data = array('level'=>$level,'currentRank'=>$currentRank,'required'=>5,'total'=>$total,'labelText'=>'Total P Member', 'labelTextRequired'=>'Required P Member Member to complete level');
