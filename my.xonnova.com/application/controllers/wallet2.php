...
* 
*/
class Wallet2 extends CI_Controller
{
	...
	function walletReportByReseller(){
		header('Content-Type: application/json');
		$id = $this->session->userdata('user_id');
		$contentData = $this->mdl_common->allSelects('SELECT * FROM  mxtopup_report_info  where user_id='.$id);
	...
	function creditwallet() {
		...
		if(!empty($id) && !empty($_POST['credit'])){
			$existUser = count($this->mdl_common->allSelects('SELECT * FROM mxtopup_report_info WHERE user_id='.$id));
			$Arr = array(
					'user_id'=>$id,
	...
	function getCreditwallet(){
		header('Content-Type: application/json');
		$getData = $this->mdl_common->allSelects('SELECT a.user_id, a.user_name, b.level, c.level_name, d.* From mxtopup_report_info as d LEFT JOIN  user_master as a on d.user_id= a.user_id LEFT JOIN earning_info as b on b.user_id=a.user_id LEFT JOIN level_configuration as c on c.l_conf_id=b.level WHERE d.wallet_type="1"');
	...
	function getDeletewallet(){
		header('Content-Type: application/json');
		$getData = $this->mdl_common->allSelects('SELECT a.user_id, a.user_name, b.level, c.level_name, d.* From mxtopup_report_info as d LEFT JOIN  user_master as a on d.user_id= a.user_id LEFT JOIN earning_info as b on b.user_id=a.user_id LEFT JOIN level_configuration as c on c.l_conf_id=b.level WHERE d.wallet_type="2"');
		if(isset($getData) && !empty($getData)){
		foreach ($getData as $key => $value) {
			$arr[] = $value;
		}
	...
	function walletReport(){
		header('Content-Type: application/json');
		$contentData = $this->mdl_common->allSelects('SELECT DISTINCT  a.*, b.* FROM user_master as a RIGHT JOIN mxtopup_report_user_map_info as b on b.user_id=a.user_id');
		if(!empty($contentData)){
			$data = '[';
			$i = 0;
			foreach ($contentData as $key => $value) {
	...
	function walletReportByUser($id){
		header('Content-Type: application/json');
		$contentData = $this->mdl_common->allSelects('SELECT * FROM user_master as a RIGHT JOIN mxtopup_report_info as b on b.user_id=a.user_id  where a.user_id='.$id);
		if(!empty($contentData)){
			foreach ($contentData as $key => $value) {
				$arr[]=$value;
			}
