  ...
  myReferrals.factory("Referrals", ['$http', function($http) {
    var base_url = BASE_URL
      ...
      obj.getReferrals = function(){
          return $http.get(base_url + 'userReferrals/getReferrals');
      };

      obj.insertUser = function (user) {
        return $http.post(base_url + 'userReferrals/insertUser', user).then(function (results) {
            return results;
        });
      };
  ...
  myReferrals.config(['$routeProvider', function($routeProvider) {
    ...
    $routeProvider
      .when('/personal-referrals', {
        templateUrl: 'application/views/user/personal-referrals.html',
        controller: 'referralsCtr'
      })
      .when('/edit-person-referrals/:userID',{
        templateUrl:'application/views/user/edit-personal-referrals.html',
